            ...
            <Flex direction={"column"} gap={"16px"}>
                {order?.items.map((item) =>
                    <Flex justifyContent={"space-between"} key={item._id}>
                        ...
                        {/* right part */}
                        <Flex direction={"column"} alignItems={"end"} gap={"5px"} width={"130px"}>
                            <Flex alignItems={"center"} gap={"8px"}>
                                {item.isAffiliate && <CartItemBadge text={`${item.percent}%`} colorScheme='green' />}
                                <AppTypography fontSize={"14px"} fontWeight={500} color={"#FFFFFF"}>{`$${item?.price?.toFixed(2)}`}</AppTypography>
                            </Flex>
                            {item.isGated &&
                            ...
                            {item.isDiscounted &&
                                <Flex alignItems={"center"} gap={"4px"}>
                                    ...
                                    <AppTypography fontSize={"10px"} color={"#FFFFFF"}>{`$${item.discountAmount?.toFixed(2)} Discount`}</AppTypography>
                                </Flex>
                            }
                        </Flex>
                    </Flex>
