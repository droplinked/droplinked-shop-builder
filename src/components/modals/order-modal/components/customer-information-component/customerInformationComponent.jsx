...
const CustomerInformationComponent = () => {
  ...
  const container = useCallback((key, name, value) => (
    <HStack key={key} color={"lightGray"}>
      <Box width={{ base: "40%", sm: "20%" }}><AppTypography fontSize="12px">{name}</AppTypography></Box>
      <Box width={{ base: "60%", sm: "80%" }}><AppTypography fontSize="12px">{value}</AppTypography></Box>
    </HStack>
  ), []);
        ...
        <VStack align={"stretch"} color="#C2C2C2">
          ...
          <VStack align="stretch" spacing="24px" paddingBottom={5}>
            ...
            <Flex alignItems="center" gap="6px" color="#808080">
              ...
              <AppTypography fontSize="12px">To track the transaction status, click on your transaction ID.</AppTypography>
            </Flex>
          </VStack>

          <VStack align="stretch" spacing="24px">
            <Box><AppTypography fontSize="16px" fontWeight="bold" color="#FFF">Customer Information</AppTypography></Box>
