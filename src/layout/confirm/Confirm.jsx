...
export default function Confirm() {
    ...
    let shopName = JSON.parse(localStorage.getItem("shopping_cart"))[0].shopName;
    let checkoutId = JSON.parse(localStorage.getItem("checkout-createdCheckout")).token;
    let customerId = JSON.parse(localStorage.getItem("checkout-customerId"));
    let cardId = JSON.parse(localStorage.getItem("checkout-selectedCard")).card.id;

    const submitPurch = () => {
        axios.post('https://dev.flatlay.io/payment',
            {
                shopName,
                checkoutId,
                customerId,
