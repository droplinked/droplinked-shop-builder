...
export default function RoleSet() {
    ...
    const addItem = () => {
        ...
        add.push(<div className="add-role-row" {...register("rulename", { required: true })}>
            <select name="" >
                <option value="and">Contract</option>
                <option value="or">NFT</option>
            </select>
            <input type="text" placeholder="NFT address" {...register("address", { required: true })} />
        </div>)
            ...
            <div className="d-flex justify-content-center">
                ...
                >
                    ...
                    <div className="role-set-page-wrap d-flex flex-column">
                        {(rules.length == 0)
                            ?
                            <div className="d-flex justify-content-center no-rule-text">No rules found</div>
                            :
                            <>
                                {rules.map((item) => {
                                    return <RoleItem name={item.name} address={item.address} type={item.type} nftType={item.nftType} />
                                })}
                            </>
                    ...
                    {(addRule) &&
                        ...
                        <div className="role-set-page-wrap d-flex flex-column" style={{ marginTop: "30px" }}>
                            <form onSubmit={handleSubmit(submitForm)}>
                                <div className="add-role-row">
                                    <select name="" placeholder="rule name" {...register("andor", { required: true })}>
                                        <option value="and">and</option>
                                        <option value="or">or</option>
                                    </select>
                                    <input type="text" placeholder="rule name" {...register("rulename", { required: true })} />
                                </div>
                                <div className="add-role-row">
                                    <select name="" {...register("nftType", { required: true })}>
                                        <option value="Contract">Contract</option>
                                        <option value="NFT">NFT</option>
                                    </select>
                                    <input type="text" placeholder="NFT address" {...register("address", { required: true })} />
                                </div>
                                {ruleCounter.map((it) => { return it })}
