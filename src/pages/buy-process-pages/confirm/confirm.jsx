...
const ConfirmPage = () => {
    ...
    const getItemsPrice = () => {
        let total = 0
        cart.forEach(item => {total += (item.amount * parseFloat(item.variant.price))})
        return total
    }
    ...
    const confirm = async() => {
        ...
        setLoading(true)
        let result = await confirmPayment(cart[0].shopName ,checkoutId.checkoutId,sessionId.sessionId)
        if(result == true){
          ...
          >
            ...
            <Box p="10px 5px" mb="50px" w={{ base: "100%", md: "100%" }}>
              ...
              <Text
                ...
                fontWeight="600"
              >
                Total price: ${(parseFloat(shippingPrice.shippingPrice))+getItemsPrice()} 
              </Text>
            </Box>

     <Flex w='100%' maxW='400px' mx='auto'>
        <BasicButton click={confirm} loading={loading}>Confirm order</BasicButton>
     </Flex>
