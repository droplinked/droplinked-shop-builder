...
function ChainAccordion({ chain }: { chain: IAuthSupportedWalletsService }) {
    ...
    const wallets = { "METAMASK": <AppIcons.MetaMaskIcon />, "UNISAT": <AppIcons.Unisat />, "METAMASK (XRPLSIDECHAIN)": <AppIcons.MetaMaskIcon />, "XVERSE": <AppIcons.Xverse />, "PHANTOM": <AppIcons.Phantom width={'24px'} height={"24px"} /> }
    ...
    const canActivateNewLoginMethod = (): boolean => {
        ...
        if (activePaymentMethods < + maxActiveLoginMethodCount) return true

        showToast({ message: AppErrors.permission.maxActiveLoginMethods(maxActiveLoginMethodCount), type: "error" })
        return false
    }
    ...
    const handleActivateWallet = useCallback((wallet: { name: string, isActivated: boolean }) => {
        ...
        if (wallet.isActivated) {
            if (!canActivateNewLoginMethod()) return
            targetChain > -1 ?
