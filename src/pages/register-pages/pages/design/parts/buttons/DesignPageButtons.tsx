...
function DesignPageButtons() {
    ...
    const { refactorLinks } = designPageButtonsModel
    ...
    return (
        <Flex justifyContent="space-between" flexDirection="row-reverse">
            <Flex gap="16px">
                ...
                <BasicButton sizes='large' isDisabled={!shop.logo.length || !shop.headerIcon.length || !shop.backgroundImage.length} isLoading={loading} onClick={submit}>{isRegister ? "Next" : "Update"}</BasicButton>
            </Flex>
            {isRegister && <BasicButton variant='outline' sizes='large' onClick={() => shopNavigate(`register/shop-info`)}>back</BasicButton>}
        </Flex>
    )
}
