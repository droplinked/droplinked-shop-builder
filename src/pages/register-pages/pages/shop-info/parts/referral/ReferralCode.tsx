...
const ReferralCode = ({ States:{ referralDetails: { code, customCode, percent } }, updateStates } : IShopInfoChildProps) => {
    ...
    const { values, handleSubmit, handleChange, setFieldValue, setFieldError, errors } = useFormik({ initialValues: { customCode }, validationSchema: object().shape({ customCode: string().matches(/^[A-Za-z0-9]+$/, 'Invalid format').required("Required") }), validateOnChange: false, onSubmit: update_referral });
