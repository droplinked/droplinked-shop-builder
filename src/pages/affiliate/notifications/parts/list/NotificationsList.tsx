...
function NotificationsList() {
	...
	const { mutate, isLoading, data } = useMutation((params: IproducerRequestService) =>
		producerRequestService(params)
	)
			...
			{isLoading ? (
				...
				<VStack align={'stretch'}>
					{list?.data.map((el: any, key: number) => {
						...
						const sku = el?.sku[0]
						const recordData = el?.recordData
						...
						return (
							...
							>
								<Flex alignItems="center" gap="4px">
									<AppTypography fontSize="12px" paddingRight="10px" color="#808080">
										From:
									</AppTypography>
									<Image src={element.logo} width="14px" height="14px" borderRadius="100%" />
									<AppTypography fontSize="12px" color="#2BCFA1">
										{element.name}
									</AppTypography>
								</Flex>
									...
									<Box>
										<VStack align="stretch" spacing="12px">
											<Flex justifyContent="space-between" gap="12px">
												<AppImage
													src={product?.media.find((el) => el.isMain === 'true')?.thumbnail}
													width="44px"
													height="44px"
													borderRadius="8px"
												/>
