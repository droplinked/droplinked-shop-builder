    ...
    <>
      {["DIGITAL", "NORMAL"].includes(product_type) && (
        <VStack align={"stretch"}>
          <ProductPageTitle
            title={
              product_type === "NORMAL" ? "Product Images" : "Product Preview"
            }
            isReuired
            description={
              product_type === "NORMAL"
                ? "Upload static images of your product."
                : "Upload images of the digital product."
            }
          />
          <Box>
            <SkeletonProduct width={"30%"} height={"200px"}>
              <InputImagesGroup
                ...
                onSuccess={(images: any) =>
                  !thumb.length &&
                  images?.small &&
                  updateState("thumb", images?.small)
                }
                setState={(images: any) =>
                  updateState("media", refactorImage(images))
                }
