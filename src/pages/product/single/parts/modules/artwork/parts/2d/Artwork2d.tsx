...
function Artwork2d() {
    ...
    const { state: { positions, printful_template_id } } = useContext(productContext)
    const { isOpen, onOpen, onClose } = useDisclosure()

    const isTechnique = useMemo(() => States?.technique || printful_template_id, [printful_template_id, States])

    useEffect(() => setStates(artwork2dStates), [isOpen])
    ...
    return (
        <artwork2dContext.Provider value={{ ...States, setStates: (key, value) => setStates(prev => ({ ...prev, [key]: value })) }} >
            ...
            {isOpen && (
                <AppModal isCentered={false} title="Create a Product Template" contentProps={{ maxWidth: isTechnique ? "1400px" : "330px", width: "95%", padding: "30px 10px" }} close={() => isTechnique ? {} : onClose()} open={true}>
