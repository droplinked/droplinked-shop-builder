...
/**
 *
 * @param {({'publicKey' : string , 'account_hash' : string , 'tokens' : [], 'signature' : string})=>{}} on_connected
...
export const casper_login = function (
  ...
  if (window.casperlabsHelper == undefined) {
    if (on_err != null) on_err(auth_err.casper_signer_not_found);
    return;
  }
          ...
          .then((publicKey) => {
            get_account_information(publicKey)
              ...
              .catch((reason) => {
                ...
                if (on_err != null) on_err(auth_err.cancelled_signing);
              });
          })
          .catch((reason) => {
