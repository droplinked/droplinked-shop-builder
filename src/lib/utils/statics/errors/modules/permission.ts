...
const permissionErrors = {
  ...
  maxActiveLoginMethods: (maxActiveLoginMethodCount: string) =>
    `You can only activate up to ${
      Number(maxActiveLoginMethodCount) === 1
        ? `${maxActiveLoginMethodCount} login  method`
        : `${maxActiveLoginMethodCount} login  methods`
    }`,
  maxActivePaymentMethods: (maxActivePaymentMethodCount: string) =>
    `You can only activate up to ${
      Number(maxActivePaymentMethodCount) === 1
        ? `${maxActivePaymentMethodCount} payment method`
        : `${maxActivePaymentMethodCount} payment methods`
    }`,
