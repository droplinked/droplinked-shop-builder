...
async function getProxyAddress(chain: Chain, network: Network) {
    const snakeCase = (str: string) => str[0].toUpperCase() + str.slice(1).toLowerCase();
    let result = String((await axios.get(`https://${network === Network.TESTNET ? "apiv3dev" : "apiv3"}.droplinked.com/storage/${snakeCase(Chain[chain])}${snakeCase(Network[network])}ContractAddressProxy`)).data.value);
    return result;
}
