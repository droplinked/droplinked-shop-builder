import { CLAccountHash, CLKey, CLPublicKey, CLString, CLU256, CLU64, CLU8, Contracts, DeployUtil, NamedArg, RuntimeArgs } from 'casper-js-sdk';
...
async function record_merch(sku_properties, account_information, product_title, price, amount, comission) {
    ...
    const signature = await window.casperlabsHelper.sign(json, producer_public_key).catch((reason) => {
        return "Cancelled";
    });
    const deployObject = DeployUtil.deployFromJson(signature);
    const deployHash = await casper_consts.casperClient.putDeploy(deployObject.val);
    return { "deploy": signature, "deployHash": deployHash };
}
export { record_merch };