...
export async function getAccountBalance(NFTContractAddress: string, address: string, network : "mainnet" | "testnet"){
    let jsonRpcProvider = network == "mainnet" ? "?" : "https://rpc-evm-sidechain.xrpl.org/";
    let provider = new ethers.providers.JsonRpcProvider(jsonRpcProvider);
 ...
 */
async function getMaxDiscount(address: string, ruleset, network : "mainnet" | "testnet"){
    let max_discount : {
